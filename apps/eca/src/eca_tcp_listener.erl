...
init([Port, Module]) ->
  ...
  case gen_tcp:listen(Port, Opts) of
    {ok, Listen_socket} ->
      ...
      {ok, Ref} = prim_inet:async_accept(Listen_socket, -1),   %% accept async
...
handle_info({inet_async, ListSock, Ref, {ok, CliSocket}},
  ...
  try
    ...
    %% supervisor.
    [_, _, _, {_, T}] = eca_tcp_sup:count_children(),
    io:format("link cnt: ~p~n", [T + 1]),
    {ok, Pid} = eca_tcp_sup:start_client(),
