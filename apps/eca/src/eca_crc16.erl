...
crc16(X) when is_binary(X) ->
    calcBin(X, 16#FFFF).

calcBin(<<H, T/binary>>, Fcs)->
    Idx = ((Fcs bsr 8) band 16#FF) bxor H,
    Fcs2 = ((Fcs bsl 8) band 16#ffff) bxor crc_index(Idx),
    calcBin(T, Fcs2);
calcBin(<<>>, Fcs) ->
    Fcs.
...
test()->
    A = "123456789",
    B = list_to_binary(A),  
    Crc16 = calcBin(B, 16#FFFF),
    io:format("crc16(~p)  is ~p. ~n",[B, Crc16]).
